...
public:
    ...
    auto run() const -> void {
        // Create attachment group that has a color attachment with resolve image for triangle rendering.
        const struct AttachmentGroup : vku::MsaaAttachmentGroup {
            AttachmentGroup(
                const vk::raii::Device &device,
                vma::Allocator allocator
            ) : MsaaAttachmentGroup { vk::Extent2D { 512, 512 }, vk::SampleCountFlagBits::e4 } {
                // Create vku::AllocateImage and move ownership into vku::AttachmentGroup.
                addColorAttachment(
                    device,
                    storeImage(createColorImage(allocator, vk::Format::eR8G8B8A8Unorm)),
                    storeImage(createResolveImage(allocator, vk::Format::eR8G8B8A8Unorm, vk::ImageUsageFlagBits::eTransferSrc)));
                setDepthAttachment(
                    device,
        ...
        const TriangleRenderer triangleRenderer {
            ...
            attachmentGroup.depthStencilAttachment->image.format,
        };
        ...
        vku::executeSingleCommand(*gpu.device, *graphicsCommandPool, gpu.queues.graphics, [&](vk::CommandBuffer commandBuffer) {
            ...
            commandBuffer.copyImageToBuffer(
                attachmentGroup.colorAttachments[0].resolveImage, vk::ImageLayout::eTransferSrcOptimal,
                destagingBuffer,
                vk::BufferImageCopy {
                    0, 0, 0,
                    vk::ImageSubresourceLayers { vk::ImageAspectFlagBits::eColor, 0, 0, 1, },
                    {}, attachmentGroup.colorAttachments[0].resolveImage.extent,
                });
        });
